...
BeepTone vRecognizeTone (image_t* img, const blobinfo_t* blobinfo, const uint8_t blobCount)
{
  ...
  {
    // Bereken centroid van huidige blob, 
    // zodat we de positie van deze weten
    vCentroid(img, i+1, &xc, &yc);
    
    yc = yc;                               // Y-positie, centrum van blob
    yb = yc + (blobinfo[i].height / 2);    // Y-positie, bovenste eind blob
    yo = yc - (blobinfo[i].height / 2);    // Y-positie, onderste eind blob
    
    // Negeer huidige blob als deze te klein (of groot) is
    ...
    else if ( (xc < SEVENSEG_START_X) && (xc < img->width) )
    {
    ...
    // Niks gevonden of ongeldige positie
    else
    {
      continue;  // Negeer, volgende blob
    }
  }
